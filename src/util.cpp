...
const char * const BITCOIN_CONF_FILENAME = "perakcoin.conf";
const char * const BITCOIN_PID_FILENAME = "perakcoin.pid";
...
#else
    const char* pszModule = "perakcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Perakcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Perakcoin";
#else
    // Unix
    return pathRet / ".perakcoin";
#endif
#endif
}
