...
*Setup instructions for a Gitian build of Perakcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Perakcoin
...
- [Building Perakcoin Core](#building-perakcoin-core)
...
Clone the git repositories for perakcoin and Gitian.
...
in the perakcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Perakcoin Core
----------------

To build Perakcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the perakcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/perakcoin/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/perakcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit perakcoin=${COMMIT} --url perakcoin=${URL} ../perakcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit perakcoin=${COMMIT} --url perakcoin=${URL} ../perakcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit perakcoin=${COMMIT} --url perakcoin=${URL} ../perakcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the perakcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../perakcoin/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url perakcoin=${BTCPATH},signature=${SIGPATH} ../perakcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/perakcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/perakcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/perakcoin-osx-build.assert
```
...
[perakcoin-project/gitian.sigs.ltc](https://github.com/perakcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
